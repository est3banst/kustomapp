...
interface FormState {
  ...
  attachments: string; // placeholder — wire to S3 upload in prod
}
...
const OptionChip: React.FC<{
  ...
  >
    ...
    {icon && (
      <span className={`mt-0.5 shrink-0 transition-colors ${selected ? "text-violet-400" : "text-gray-700 group-hover:text-gray-500"}`}>
        {icon}
      </span>
    )}
    ...
    {selected && (
      <span className="absolute top-2.5 right-2.5 w-3.5 h-3.5 rounded-full bg-violet-500 flex items-center justify-center shrink-0">
      ...
      <div className={`flex flex-wrap gap-2 min-h-[48px] px-3 py-2.5 bg-violet-950/20 border border-violet-900/50 focus-within:border-violet-500/70 rounded transition-colors`}>
        {skills.map((s) => (
          <span key={s} className="flex items-center gap-1.5 px-2.5 py-1 rounded bg-violet-900/50 border border-violet-700/50 text-[11px] font-semibold text-violet-200">
            {s}
            <button type="button" onClick={() => remove(s)} className="text-violet-500 hover:text-violet-300 transition-colors leading-none">×</button>
          </span>
        ))}
        <input
          value={input}
...
const PreviewCard: React.FC<{ form: FormState; lang: string; postedAt: string }> = ({ form, lang, postedAt }) => (
  <div className="sticky top-24 flex flex-col gap-4 p-6 border border-violet-900/40 bg-violet-950/10 rounded-xl overflow-hidden">
    ...
    {form.skills.length > 0 && (
      <div className="flex flex-wrap gap-1.5">
        {form.skills.map((s) => (
          <span key={s} className="px-2 py-0.5 text-[9px] font-bold rounded border border-violet-900/50 bg-violet-950/20 text-gray-500">
            {s}
          </span>
        ))}
      </div>
    ...
    <div className="flex flex-col gap-1.5 pt-1 border-t border-violet-900/30">
      {form.budget && (
        <div className="flex items-center justify-between text-[10px]">
          ...
          <span className="font-semibold text-gray-400">{form.budget}</span>
        </div>
      )}
      {form.timeline && (
        <div className="flex items-center justify-between text-[10px]">
          <span className="text-gray-600">{lang === "en" ? "Timeline" : "Plazo"}</span>
          <span className="font-semibold text-gray-400">{form.timeline}</span>
        </div>
      )}
      <div className="flex items-center justify-between text-[10px]">
        ...
        <span className={`font-semibold ${form.visibility === "public" ? "text-emerald-400" : "text-amber-400"}`}>
          {form.visibility === "public" ? (lang === "en" ? "Public" : "Público") : (lang === "en" ? "Invite only" : "Solo por invitación")}
        </span>
      </div>
    </div>
  </div>
...
const PostProject: React.FC = () => {
  ...
  const t = {
    ...
    title:        lang === "en" ? "Describe what you need"   : "Describí lo que necesitás",
    sub:          lang === "en"
      ? "The more detail you provide, the better the proposals you'll receive."
      : "Cuanto más detalle brindás, mejores propuestas vas a recibir.",
    ...
    titleLabel:   lang === "en" ? "Project title"            : "Título del proyecto",
    titlePH:      lang === "en" ? "e.g. E-commerce store for a clothing brand" : "ej. Tienda online para marca de ropa",
    catLabel:     lang === "en" ? "Project category"         : "Categoría del proyecto",
    s2title:      lang === "en" ? "Project description"      : "Descripción del proyecto",
    s2sub:        lang === "en"
      ? "Describe the problem you're trying to solve, what you want built, and any technical context."
      : "Describí el problema que querés resolver, qué querés construir y cualquier contexto técnico.",
    descLabel:    lang === "en" ? "Description"              : "Descripción",
    descPH:       lang === "en"
      ? "Tell developers what you need built, any existing systems to integrate with, design preferences, or specific requirements..."
      : "Contales a los desarrolladores qué necesitás construir, con qué sistemas existentes hay que integrarse, preferencias de diseño o requisitos específicos...",
    skillsLabel:  lang === "en" ? "Required skills"          : "Habilidades requeridas",
    s3title:      lang === "en" ? "Scope & timeline"         : "Alcance y plazo",
    s3sub:        lang === "en" ? "Help developers understand the size and urgency of your project." : "Ayudá a los desarrolladores a entender el tamaño y la urgencia de tu proyecto.",
    budgetLabel:  lang === "en" ? "Budget range"             : "Rango de presupuesto",
    timelineLabel:lang === "en" ? "Timeline"                 : "Plazo estimado",
    s4title:      lang === "en" ? "Visibility"               : "Visibilidad",
    s4sub:        lang === "en" ? "Choose who can see and respond to your project." : "Elegí quién puede ver y responder a tu proyecto.",
    submit:       lang === "en" ? "Post project"             : "Publicar proyecto",
    submitting:   lang === "en" ? "Publishing..."            : "Publicando...",
    doneTitle:    lang === "en" ? "Project posted!"          : "¡Proyecto publicado!",
    doneSub:      lang === "en"
      ? "Your project is now live. Developers will start sending proposals shortly."
      : "Tu proyecto ya está publicado. Los desarrolladores comenzarán a enviarte propuestas pronto.",
  ...
  const reset = () => {
    setForm({ title: "", category: null, description: "", skills: [], budget: null, timeline: null, visibility: "public", attachments: "" });
    setErrors({});
    setDone(false);
  };
    ...
    return (
      <>
        ...
        <div className="min-h-screen bg-black pt-24 pb-16 flex items-center justify-center relative overflow-hidden px-6">
          ...
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[500px] h-[500px] rounded-full bg-violet-700 opacity-[0.06] blur-[100px] pointer-events-none" />
          <div className="relative z-10 max-w-md w-full text-center flex flex-col items-center gap-8">
            ...
            <div className="w-full p-5 border border-violet-900/40 bg-violet-950/10 rounded-xl text-left flex flex-col gap-3">
              <div className="text-xs font-black text-white">{form.title}</div>
              <div className="flex items-center gap-3 flex-wrap">
                {form.category && (
                  <span className="px-2 py-0.5 rounded text-[9px] font-black tracking-widest uppercase border border-violet-700/50 bg-violet-950/30 text-violet-400">{form.category}</span>
                )}
                {form.budget && (
                  <span className="text-[10px] text-gray-500">{form.budget}</span>
                )}
                <span className="text-[10px] text-gray-600 ml-auto">{postedAt}</span>
              </div>
            </div>
            <div className="flex flex-col sm:flex-row gap-3 w-full">
              <Link to="/user-page" className="flex-1">
              ...
              <button onClick={reset} className="flex-1 py-3 border border-violet-700/50 hover:border-violet-500 text-violet-300 font-black text-xs tracking-[0.2em] uppercase rounded transition-all">
                {t.postAnother}
              </button>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-10">
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-12" noValidate>
              ...
              <Section number="01" title={t.s1title} sub={t.s1sub}>
                <Field label={t.titleLabel} required hint={`${form.title.length}/100`} error={errors.title}>
                  <Input
                    value={form.title}
                ...
                <Field label={t.catLabel} required error={errors.category}>
              ...
              <Section number="02" title={t.s2title} sub={t.s2sub}>
                <Field label={t.descLabel} required hint={`${form.description.length}/2000`} error={errors.description}>
                  <Textarea
              ...
              <Section number="03" title={t.s3title} sub={t.s3sub}>
                <Field label={t.budgetLabel} required error={errors.budget}>
                ...
                <Field label={t.timelineLabel} required error={errors.timeline}>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    {timelines.map((tl) => (
                      <OptionChip
                        ...
                        selected={form.timeline === tl.value}
                        onClick={() => set("timeline", tl.value)}
                      />
                    ))}
                  </div>
                </Field>
              ...
              <Section number="04" title={t.s4title} sub={t.s4sub}>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <OptionChip
                    ...
                    selected={form.visibility === "invite"}
                    onClick={() => set("visibility", "invite")}
                    icon={<svg xmlns="http://www.w3.org/2000/svg" width={14} height={14} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.5}><rect x={3} y={11} width={18} height={11} rx={2} ry={2}/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>}
                  />
                </div>
              </Section>
              ...
              <div className="flex flex-col sm:flex-row gap-4 items-center pt-2 pb-12">
                ...
                <Link to="/user-page" className="w-full sm:w-auto text-center text-xs font-semibold tracking-widest uppercase text-gray-600 hover:text-gray-400 transition-colors py-2">
